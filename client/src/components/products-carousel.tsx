...
// Updated Validation Schema: Email Removed
...
export function ProductsCarousel() {
 ...
 /* const submitLead = useMutation({
  ...
  const next = () => { setCurrentIndex((prev) => (prev + 1) % products.length); setSpringType(0); };
  const prev = () => { setCurrentIndex((prev) => (prev - 1 + products.length) % products.length); setSpringType(0); };
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                  ...
                  <DialogContent className="sm:max-w-[450px] rounded-3xl p-8 border-none shadow-2xl">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Capturing requirements for <span className="font-bold text-[#001c44]">{current.name}</span>.
                      </DialogDescription>
                    </DialogHeader>
                    ...
                    <Form {...form}>
                      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
                        <FormField control={form.control} name="name" render={({ field }) => (
                          <FormItem><FormLabel className="text-[10px] uppercase font-black text-slate-400">Contact Name</FormLabel>
                        ...
                        <FormField control={form.control} name="hotelName" render={({ field }) => (
                          <FormItem><FormLabel className="text-[10px] uppercase font-black text-slate-400">Hotel Name</FormLabel>
...
/>
                        ...
                        <Button type="submit" disabled={submitLead.isPending} className="w-full h-14 bg-amber-500 hover:bg-amber-600 text-[#001c44] font-black uppercase tracking-widest text-xs rounded-xl flex gap-2 mt-4">
